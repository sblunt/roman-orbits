...
anybasis_params["w2"] = radvel.Parameter(value=0.1)  # argument of periastron of the star's orbit for 2nd planet
...
anybasis_params["gamma_ELODIE"] = radvel.Parameter()  # velocity zero-point
anybasis_params["gamma_AFOE"] = radvel.Parameter()
anybasis_params["gamma_SOPHIE"] = radvel.Parameter()
anybasis_params["gamma_Hamilton"] = radvel.Parameter()
anybasis_params["gamma_HIRES-pre"] = radvel.Parameter()
anybasis_params["gamma_HIRES-post"] = radvel.Parameter()
anybasis_params["gamma_APF"] = radvel.Parameter()

anybasis_params["jit_ELODIE"] = radvel.Parameter(value=)  # jitter
anybasis_params["jit_AFOE"] = radvel.Parameter(value=)
anybasis_params["jit_SOPHIE"] = radvel.Parameter(value=)
anybasis_params["jit_Hamilton"] = radvel.Parameter(value=)
anybasis_params["jit_HIRES-pre"] = radvel.Parameter(value=)
anybasis_params["jit_HIRES-post"] = radvel.Parameter(value=)
anybasis_params["jit_APF"] = radvel.Parameter(value=)
...
path = "/Users/macuser/Roman/rv_orbits/roman-orbits/all_fits/HD_95735/data/HD95735_rv_combined_binned.txt"
data = pd.read_csv(
    path,
    header=None,
    skiprows=1,
